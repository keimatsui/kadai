...
* Gitクライアントは，GitHub Desktop（GitHub公式）とSourcetree（ユーザー登録が必要だが高機能）を入れている。
* Rは標準のものよりも高速なMicrosoft R Openを使う。（標準のRを使うなら，`cinst -y r.project`）

```
cinst -y chrome curl firefox git github notepad2 microsoft-r-open r.studio rsync sourcetree vagrant virtualbox wget
```
