...
* テキストエディタはNotepad2と[Visual Studio Code](https://code.visualstudio.com/download)を推奨する。Notepad2は本格的に使うわけではなく，メモ帳を置き換えるために入れている。
* ChocolateyのJavaには問題があるため入れていない。（学生には関係ないが，大学の会計システムが動かない。）
* Gitクライアントは，GitHub Desktop（GitHub公式）とSourcetree（ユーザー登録が必要だが高機能）を入れている。
* Rは，標準のもの（r.project）とMicrosoft R Openを入れている。RStudioで切り替えて使える。

```
cinst -y chrome curl firefox git github notepad2 microsoft-r-open r.project r.studio rsync sourcetree vagrant virtualbox visualstudiocode wget
```
