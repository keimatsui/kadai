...
1. ユーザ名「`yabuki`」，パスワード「`yabuki`」で接続する。
...
mkdir share
sudo mount -t cifs //10.100.192.3/share share -o user=yabuki
```
...
* テキストエディタはNotepad2と[Visual Studio Code](https://code.visualstudio.com/download)を推奨する。Notepad2は本格的に使うわけではなく，メモ帳を置き換えるために入れている。Visual Studio CodeはAnacondaインストール時に一緒にインストールできる（Chocolateyで入れるなら，`cinst -y visualstudiocode`）。
* Chocolateyで入るJavaには，大学の会計システムが動かないという問題がある。
* Sourcetreeは無料だが，利用のためにはユーザー登録が必要である。これの代わりにGitHub Desktopを使ってもいい（Chocolateyで入れるなら，`cinst -y github`）。
* Rは標準のものよりも高速なMicrosoft R Openを使う。

```
cinst -y chrome curl git github notepad2 microsoft-r-open r.studio rsync sourcetree vagrant virtualbox wget
```
