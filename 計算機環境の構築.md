...
有名なソフトウェアはだいたいChocolateyで入れられる。たとえば，コマンドプロンプト（管理者）で以下を実行すれば，Windows上で必要なものはだいたい揃う。（Pythonは例外で，[Anaconda](https://www.continuum.io/downloads)を入れたほうがいいだろう。仮想マシンに入れてもいい。）
...
インストール済みのパッケージは`choco list -lo`で確認する。パッケージのアンインストールは`cup パッケージ名`あるいは`cup all`，パッケージのアンインストールは`cuninst パッケージ名`。
