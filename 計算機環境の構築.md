...
本格的に使うなら，（Chocolateyで入れられる）TeraTermなどの専用クライアントが便利かもしれない。その場合のパスワードは`C:\Users\<username>\.vagrant.d\boxes\ubuntu-VAGRANTSLASH-xenial64\20XXMMDD.0.0\virtualbox\Vagrantfile`に記載がある（`passwd ubuntu`で変更可）。`127.0.0.1`（ポート`2222`）にユーザ名`ubuntu`，パスワード`(上記設定値)`で接続する。
