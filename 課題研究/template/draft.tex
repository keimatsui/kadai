...
図は，できる限りベクタ形式で作る。ラスタ形式でもよいのは，写真を画面キャプチャである。Excelで作ったグラフやPowerPointで作った図は，プリンタ「Adobe PDF」で印刷する。そうしてできるPDFファイルをIllustratorで読み込み，向きを修正（対象を選択→オブジェクト→変形→回転），アートボードを対象に合わせる（オブジェクト→アートボード→オブジェクト全体に合わせる）。
